...
class AppointmentService:
    ...
    def get_appointment(tenant_id: str, appointment_id: str) -> Optional[AppointmentInDB]:
        ...
        response = db.table("appointments").select("*").eq("tenant_id", tenant_id).eq("id", appointment_id).execute()
        if response.data:
    ...
    def update_appointment_status(tenant_id: str, appointment_id: str, status: AppointmentStatus) -> Optional[AppointmentInDB]:
        ...
        response = db.table("appointments").update(update_data).eq("tenant_id", tenant_id).eq("id", appointment_id).execute()
    ...
    def get_all_appointments(tenant_id: str) -> List[AppointmentInDB]:
        ...
        response = db.table("appointments").select("*").eq("tenant_id", tenant_id).execute()
        if response.data:
